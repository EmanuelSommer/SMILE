...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["PredPerf", "Lppd", "Calibration", "BasicConfig"],
    ...
    parser.epilog = "Example usage: python pool_results.py --dir /path/to/dir --filter 'exp1' --what 'PredPerf' 'Lppd' 'Calibration' --warmstart --verbose True --output pooled_results.json"
...
def main():
    ...
    for d in filtered_dirs:
        ...
        if not os.path.exists(eval_dir):
            if args.verbose:
                print(f"Skipping {d} - no eval directory")  # noqa: T201
            continue
        subdirs = os.listdir(eval_dir)
