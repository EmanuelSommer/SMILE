...
def make_L_step_size_adaptation(
    ...
    def predictor(
        ...
        # step_number_f = jnp.array(step_number, dtype=jnp.float32)
        # logdensity_grad_leaves, _ = jax.tree_util.tree_flatten(state.logdensity_grad)
        # logdensity_grad_value = jnp.concatenate(
        #     [leaf.flatten() for leaf in logdensity_grad_leaves]
        # )
        # # heavy clipping!
        # logdensity_grad_value = jnp.clip(logdensity_grad_value, -1e2, 1e2)
        # epsilon = 1e-3

        # decay_factor = 0.999
        # # For now not using bias correction (only holds for 0 init anyway?)
        # adjusted_step_number = step_number_f - tune1_steps + 1

        # new_sqrt_diag_cov = jnp.sqrt(
        #     (
        #         decay_factor * params.sqrt_diag_cov**2
        #         + (1.0 - decay_factor) * logdensity_grad_value**2
        #     )
        #     + epsilon
        #     # ) / (
        #     #     1.0 - jnp.power(
        #     #         decay_factor,
        #     #         jnp.maximum(adjusted_step_number, 1.0),
        #     #     ) + epsilon
        #     # )
        # )
        # # only for phase 2
        # # new_sqrt_diag_cov = jnp.where(
        # #     step_number_f < tune1_steps,
        ...
        # #     ),
        # # )
        # # Clip new_sqrt_diag_cov to avoid extreme values
        # new_sqrt_diag_cov = jnp.clip(new_sqrt_diag_cov, 1e-7, 1e7)
        # new_sqrt_diag_cov = jnp.where(
        #     jnp.isnan(new_sqrt_diag_cov), params.sqrt_diag_cov, new_sqrt_diag_cov
        # )
        # params = params._replace(sqrt_diag_cov=new_sqrt_diag_cov)
