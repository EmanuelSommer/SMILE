...
import src.inference.metrics as bm_metrics
...
from src.inference.evaluation import EvaluationName, Evaluator
from src.inference.metrics import (
    ClassificationMetrics,
    MetricsStore,
    RegressionMetrics,
)
from src.inference.predict import sample_from_predictions
from src.inference.sample_loader import SampleLoader
from src.training.sampling import inference_loop, inference_loop_batch
from src.training.utils import earlystop, get_nn_size
...
class BDETrainer:
    ...
    def __init__(self, config: Config, folder: Optional[Path] = None):
        ...
        match self.config.data.data_type:
            case DatasetType.TEXT:
                from src.config.training import TokenizerName
                from src.dataset.tokenizers import (  # noqa
                    BPETokenizer,
                    CustomBPETokenizer,
                    SingleCharTokenizer,
                )

                tokenizer: Union[SingleCharTokenizer, CustomBPETokenizer, BPETokenizer]

                if self.config.training.tokenizer.tokenizer == TokenizerName.SingleChar:
                    tokenizer = SingleCharTokenizer()
                elif (
                    self.config.training.tokenizer.tokenizer == TokenizerName.CustomBPE
                ):
                    if not (
                        "context_len" in self.config.model
                        and "vocab_size" in self.config.model
                    ):
                        raise ValueError(
                            "Missing fields in the model config definition."
                            'Add "context_len" and "vocab_size" fields '
                            f"to the model config definition. {self.config.model.__class__.__name__}"
                        )
                    tokenizer = CustomBPETokenizer(
                        pad_len=self.config.model["context_len"],
                        max_vocab_size=self.config.model["vocab_size"],
                    )
                elif self.config.training.tokenizer.tokenizer == TokenizerName.BPE:
                    tokenizer = BPETokenizer()
                else:
                    raise ValueError(
                        f"Tokenizer {self.config.training.tokenizer.tokenizer} not implemented."
                    )
                self.loader = TextLoader(
                    ...
                    tokenizer=tokenizer,
                    context=self.config.model["context_len"],
                    omit_freq=0,  # no omitting for now
                )
        ...
        self.loader.save_splits_to_disk(path=self.exp_dir / "dataset_splits")

        # Setup Probabilistic Model
        logger.info("> Setting up Probabilistic Model...")
        self.module = self.config.get_flax_model()
        self.batch_size_s = self.config_sampler.batch_size or self.loader.data_train_len

        if self.config_sampler.batch_size is None:
            self.batch_size_s_test = self.loader.data_test_len
            self.batch_size_s_valid = self.loader.data_valid_len
        else:
            self.batch_size_s_test = min(
                self.config_sampler.batch_size, self.loader.data_test_len
            )
            self.batch_size_s_valid = min(
                self.config_sampler.batch_size, self.loader.data_valid_len
            )

        self.batch_size_w = (
            self.config_warmstart.batch_size or self.loader.data_train_len
        )
        self.n_batches = (self.loader.data_train_len // self.batch_size_s) or 1
            ...
            if cfg_warm.warmstart_exp_dir:  # With checkpoint
                ...
                if cfg_warm.permutation_warmstart:
                    ...
                    permuted_params = train_utils.permute_warmstart(
                        warm_path, n_chains=self.n_chains, base_param=0, key=self.key
                    )
    ...
    def start_sampling(self):
        ...
        for step in self.train_plan:
            ...
            if self.config_sampler.is_minibatch:  # Mini-Batch Sampling
                log_post = partial(self.prob_model.log_unnormalized_posterior)
                inference_loop_batch(
                    grad_estimator=jax.value_and_grad(log_post, allow_int=True),
    ...
    def evaluate(self):
        ...
        if EvaluationName.PRIOR_LOGLIK in self.config.evaluations:
            prior_loglik_split = self.config.evaluation_args.get(
                "prior_loglik_split", "test"
            )
            if prior_loglik_split == "test":
                data_batch_size_prior_loglik = self.batch_size_s_test
            elif prior_loglik_split == "valid":
                data_batch_size_prior_loglik = self.batch_size_s_valid
            else:
                raise NotImplementedError(
                    f"Prior loglik split {prior_loglik_split} not implemented."
                )
            ...
            for phase in phases_pre_pred:
                evaluators[EvaluationName.PRIOR_LOGLIK].evaluate_and_save(
                    ...
                    data_batch_size=data_batch_size_prior_loglik,
                    prob_model=self.prob_model,
                    split=prior_loglik_split,
                )
        try:
            datapoint_limit_pred = self.config.evaluation_args["datapoint_limit_pred"]
        except KeyError:
            datapoint_limit_pred = None
        if EvaluationName.PRED_PERF_GPT in self.config.evaluations:
            for phase in phases:
                evaluators[EvaluationName.PRED_PERF_GPT].evaluate_and_save(
                    phase=phase,
                    train_plan=self.train_plan,
                    sample_loader=sample_loader,
                    sample_batch_size=self.config.evaluation_args["samples_batch_size"],
        ...
        if EvaluationName.PREDICT in self.config.evaluations:
            ...
            eval_thinning = self.config.evaluation_args.get("eval_thinning", 1)
    ...
    def train_de_member(
        ...
        match self.task:
            ...
            case Task.CLASSIFICATION_MULTILABEL:
        ...
        for epoch in range(self.config_warmstart.max_epochs):
            ...
            for i, batch in enumerate(
                ...
                rng_keys = jax.random.split(self.key, batch[0].shape[0])
                batch_x = jax.vmap(self.loader.augment)(batch[0], rng_key=rng_keys)
                state, metrics = jax.pmap(step_func)(state, batch_x, batch[1], _stop_n)
        ...
        # Note: Is evaluated under evaluation
        # for batch in self.loader.iter(split="test", chains=chains):
        #     metrics = jax.pmap(pred_func, in_axes=(0, 0, 0, None))(
        #         state, batch[0], batch[1], False
        #     )
