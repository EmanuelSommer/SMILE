...
from src.bnns.logliks import ProbabilisticModel
from src.config.data import Task
from src.dataset.base import BaseLoader
from src.inference.sample_loader import SampleLoader
...
class EvaluationName(str, Enum):
    ...
    Attributes:
        ...
        PRIOR_LOGLIK: Log-likelihood and Log Prior of the samples.
    """

    LPPD = "Lppd"
    LPPD_CUM = "LppdCum"
    PRED_PERF = "PredPerf"
    PRED_PERF_GPT = "PredPerfGPT"
    CRHAT = "Crhat"
    ESS = "Ess"
    CHAIN_VAR = "ChainVar"
...
class Lppd(Evaluator):
    ...
    def evaluate_and_save(
        ...
        # last chainwise lppd
        if len(chainwise_running_lppd.shape) == 2:
            results["chainwise_lppd"] = chainwise_running_lppd[:, -1].tolist()
        running_lppd = metrics.running_lppd(lppd_pointwise=lppd_pointwise)
        running_seq_chain_lppd = metrics.running_seq_chain_lppd(
            lppd_pointwise=lppd_pointwise
        )
        ...
        self.save_results_as_npz(
            {
                ...
                "running_seq_chain_lppd": running_seq_chain_lppd,
            },
            phase,
        )
...
class LppdCum(Evaluator):
    ...
    def evaluate_and_save(
        ...
        #     results={
        #         "mean_seq_lppd": mean.tolist(),
        #         "std_seq_lppd": sd.tolist(),
        #     },
        #     phase=phase,
        # )
    ...
    def evaluate_and_save(
        ...
        match self.task:
            case Task.REGRESSION | Task.MEAN_REGRESSION:
                ...
                results["chainwise_rmse"] = [
                    metrics.rmse(
                        pred=jnp.expand_dims(pred_dist[c, ..., 0], axis=0),
                        target=target,
                    ).item()
                    for c in range(pred_dist.shape[0])
                ]
            case Task.CLASSIFICATION | Task.CLASSIFICATION_MULTILABEL:
                ...
                results["brier"] = metrics.brier_score(
                    pred_dist=pred_dist, target=target
                ).item()
                results["nll"] = metrics.negative_log_likelihood(
                    pred_dist=pred_dist, target=target
                ).item()
                results["auroc"] = metrics.auroc(
                    pred_dist=pred_dist, target=target
                ).item()
                results["aurc"] = metrics.aurc(
                    pred_dist=pred_dist, target=target
                ).item()
...
class PredPerfGPT(Evaluator):
    """Predictive performance evaluation for GPT style models.

    In particular we want the accuracy over the full context length based on the target
    vs the majority vote over all samples and chains AND the perplexity of the ensemble.
    """

    name = EvaluationName.PRED_PERF_GPT
    post_predict: bool = False
    ...
    def evaluate_and_save(
        ...
        **kwargs,
        ...
        eval_n = 0
        nll_sum = 0.0
        all_chain_indices = sorted([idx for grp in train_plan for idx in grp])
        chain_hits = {idx.item(): 0 for idx in all_chain_indices}
        chain_nll_sum = {idx.item(): 0.0 for idx in all_chain_indices}
        ...
        for x, y in data_loader.iter(
            ...
            x = jnp.repeat(x, len(train_plan[0]), axis=0)
            preds_dist_l1 = []
            for chains in train_plan:
                ...
                for samples in sample_loader.iter(
                    ...
                    preds_dist = jax.pmap(predict_from_samples)(
                        samples, x
                    )  # dimensions (chains, samples, nobs, context, logits)
                    preds_dist = jnp.reshape(
                        preds_dist,
                        (
                            preds_dist.shape[0],
                            preds_dist.shape[1],
                    ...
                    preds_dist_chain.append(preds_dist)
                preds_dist_chain = jnp.concatenate(preds_dist_chain, axis=1)
                for i, chain_idx in enumerate(chains):
                    single_chain_pred_dist = preds_dist_chain[i]
                    vocab_size = data_loader.tokenizer.vocab_size  # type: ignore[attr-defined]
                    chain_preds = jnp.argmax(single_chain_pred_dist, axis=-1)
                    chain_majority_vote = jax.vmap(
                        lambda col: jnp.bincount(col, length=vocab_size).argmax()
                    )(chain_preds.T)
                    chain_hits[chain_idx.item()] += jnp.sum(chain_majority_vote == y)
                    chain_log_probs = jax.scipy.special.log_softmax(
                        single_chain_pred_dist, axis=-1
                    )
                    chain_log_avg_probs = jax.scipy.special.logsumexp(
                        chain_log_probs, axis=0
                    ) - jnp.log(chain_log_probs.shape[0])
                    chain_true_class_log_probs = chain_log_avg_probs[
                        jnp.arange(chain_log_avg_probs.shape[0]), y.astype(int)
                    ]
                    chain_nll_sum[chain_idx.item()] += -jnp.sum(
                        chain_true_class_log_probs
                    )

                preds_dist_l1.append(preds_dist_chain)
            preds_dist_l1 = jnp.concatenate(preds_dist_l1, axis=0)
            preds = jnp.argmax(preds_dist_l1, axis=-1)  # (chains, samples, merged_obs)
            preds = preds.reshape(-1, preds.shape[-1])  # (chains*samples, merged_obs)
            vocab_size = data_loader.tokenizer.vocab_size  # type: ignore[attr-defined]
            majority_vote = jax.vmap(
                lambda col: jnp.bincount(col, length=vocab_size).argmax()
            )(preds.T)
            hits += jnp.sum(majority_vote == y)
            eval_n += y.shape[0]
            log_probs = jax.scipy.special.log_softmax(preds_dist_l1, axis=-1)
            log_avg_probs = jax.scipy.special.logsumexp(
                log_probs, axis=(0, 1)
            ) - jnp.log(preds_dist_l1.shape[0] * preds_dist_l1.shape[1])  # type: ignore[attr-defined]
            true_class_log_probs = log_avg_probs[
                jnp.arange(log_avg_probs.shape[0]), y.astype(int)
            ]
            nll_sum += -jnp.sum(true_class_log_probs)

        results = {}
        results["accuracy"] = (hits / eval_n).item()  # type: ignore[attr-defined]
        results["perplexity"] = jnp.exp(nll_sum / eval_n).item()

        if eval_n > 0:
            # Sort by key to ensure output is ordered: [metric_for_chain_0, metric_for_chain_1, ...]
            sorted_keys = sorted(chain_hits.keys())
            results["chain_accuracy"] = [
                (chain_hits[k] / eval_n).item()  # type: ignore[attr-defined]
                for k in sorted_keys
            ]
            results["chain_perplexity"] = [
                jnp.exp(chain_nll_sum[k] / eval_n).item() for k in sorted_keys
            ]
        else:
            results["chain_accuracy"] = [0.0] * len(all_chain_indices)
            results["chain_perplexity"] = [float("inf")] * len(all_chain_indices)
        ...
        **kwargs,
        ...
        for chains in train_plan:
            ...
            for samples in sample_loader.iter(
                ...
                if eval_thinning > 1:
                    preds_chain = preds_chain[:, ::eval_thinning, ...]
...
class Calibration(Evaluator):
    ...
    def evaluate_and_save(
        ...
        self.log(phase)
        if self.task in [Task.CLASSIFICATION, Task.CLASSIFICATION_MULTILABEL]:
