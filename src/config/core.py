"""Top level configuration class for the src package."""
...
from src.inference.evaluation import EvaluationName
...
class Config(BaseConfig):
    ...
    Note:
        - This is the top level configuration class for the src package.
    ...
    Example:
        You can create a Template for specific model and fill-out the template::
    ...
    def to_template(cls, model: str | nn.Module):
        ...
        Args:
            model: The model to get the template for. If the model is pre-defined
                in the `__init__.py` file of the `src.architectures` submodule,
                it can be referenced by a string name. Otherwise, pass the model
                class directly to generate the template.

        Notes:
            - Each Machine Learning Model in the `src` framework must have a
            `config` field with the corresponding `ModelConfig` subclass as its type.
            This subclass contains the configurations the user wants to configure from the
            configuration file. Next to the `config` field, the user can add as many
            extra fields as needed, as long as they have default values.
    ...
    def list_avaliable_models():
        ...
        Note:
            - Models must be defined in the __init__.py file of `src.architectures`.
        """
    ...
    def _get_model_config_cls(model: nn.Module) -> type[ModelConfig]:
        ...
        Notes:
            ...
            - Each Machine Learning Model in the `src` franemwork has a
